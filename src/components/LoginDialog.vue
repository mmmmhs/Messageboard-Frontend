<template>
    ...
    >
        ...
        <el-form :model="loginDialog.form" label-width="80px">
            <span style="color: red" v-if="loginDialog.dialogError !== ''">{{
                loginDialog.dialogError
            }}</span>
            ...
            <el-form-item label="用户名">
                <el-input
                    v-model="loginDialog.form.username"
                    :aria-placeholder="state"
                />
            </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
            <el-button v-on:click="close_dialog">返回</el-button>
            <el-button
                type="primary"
...
export default {
    ...
    props: {
        ...
        updateUsername: {
            //外部传入回调函数,父组件的变量只能由父组件自己修改,子组件不能修改父组件传入的props
            type: Function,
			default: () => {}
        },
		close_dialog: {
    ...
    methods: {
        setUserName() {
			...
			CookieOperation.setCookie("user", this.loginDialog.form.username, 365 * 24 * 60 * 60 * 1000)
			this.updateUsername()
			this.close_dialog()
        },
    },
